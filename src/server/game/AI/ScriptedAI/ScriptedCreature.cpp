...
void ScriptedAI::SetEquipmentSlots(bool loadDefault, int32 mainHand /*= EQUIP_NO_CHANGE*/, int32 offHand /*= EQUIP_NO_CHANGE*/, int32 ranged /*= EQUIP_NO_CHANGE*/)
{
    ...
    if (mainHand >= 0)
        me->SetVirtualItem(VIRTUAL_ITEM_SLOT_0, uint32(mainHand));

    if (offHand >= 0)
        me->SetVirtualItem(VIRTUAL_ITEM_SLOT_1, uint32(offHand));

    if (ranged >= 0)
        me->SetVirtualItem(VIRTUAL_ITEM_SLOT_2, uint32(ranged));
}
